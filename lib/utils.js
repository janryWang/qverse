...
var testOpts = function testOpts(opts, params, matcher) {
  if (opts === void 0) {
    opts = {};
  }

  var _path = toArr(params.path);
  ...
  if (isArr(opts.include)) {
    return opts.include.some(function (path) {
      return (0, _dotMatch.default)(path)(_path);
    }) && testOpts({
      exclude: opts.exclude
    }, params, matcher);
  }
  ...
  return function (params) {
    ...
    if (isArr(path)) {
      return path.some(function (p) {
        return createMatcher(p, options, (0, _dotMatch.default)(p))(params);
      });
...
var excute = function excute(payload, actions, params) {
  return actions.reduce(function (buf, action) {
    if (action.handler) {
      return action.matcher(params) && isNotEmptyPath(params.path) ? action.handler(buf) : buf;
    } else if (action.rescue) {
      return action.matcher(params) && isNotEmptyPath(params.path) ? payload : buf;
    }
